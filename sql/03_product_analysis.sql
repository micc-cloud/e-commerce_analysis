...
CREATE OR REPLACE VIEW amazon_category_performance_scoped AS
WITH category_totals AS (
    SELECT
...
SELECT
    *,
    RANK() OVER (PARTITION BY analysis_scope ORDER BY reported_amount DESC NULLS LAST) AS amount_rank
FROM category_totals;

CREATE OR REPLACE VIEW amazon_sku_performance_scoped AS
WITH sku_totals AS (
    SELECT
        'reported_source' AS analysis_scope,
        sku,
    ...
    SELECT
        ...
        SUM(CAST(amount AS DOUBLE)) AS reported_amount
    FROM amazon_status_scoped_sales
    WHERE sku IS NOT NULL AND is_delivered_status_proxy
    GROUP BY 1, 2
)
SELECT
    *,
    RANK() OVER (PARTITION BY analysis_scope ORDER BY reported_amount DESC NULLS LAST) AS amount_rank_desc,
    RANK() OVER (PARTITION BY analysis_scope ORDER BY reported_amount ASC NULLS LAST) AS amount_rank_asc
FROM sku_totals;
